This version uses **GitHub Flavored Markdown (GFM)** to turn your text into a clean, professional, and readable project page. It utilizes bold headers, blockquotes for the legal section, and a clear step-by-step layout.

Copy and paste the following into your `README.md` file:

---

# 🛰️ Wikelo Cloud Sync v1.0

**Wikelo Cloud Sync** is a real-time, shared-authority logistics and inventory management tool. It transforms a standard Google Sheet into a live, collaborative database, allowing squad members to coordinate materials, locations, and contract completions in sync.
...
1. Open `index.html`.
2. Find the `const SCRIPT_URL` variable at the beginning of the `<script>` block.
3. Replace the placeholder with your **Web App URL**.
4. Ensure `wikelo-data.json` and `locations.json` are in the same folder as `index.html`.

---

## 🎮 How to Play

1. **Join the Squad**: Enter a `SQUAD_CODE` (must be identical for all teammates) and your `HANDLE`.
2. **Log Materials**: Add items via the sidebar. Use **MOV** to update a custom location or **WKL** to send them to a primary hub.
3. **Monitor Contracts**: The main grid displays recipes. When a contract turns **Green**, it is **READY** for hand-in.
4. **Complete Mission**: Click **INITIALIZE_HAND_IN**, select the source hub, and the tool will consume the materials and log the victory in the **Recent Transactions** feed.
...
**Would you like me to help you generate a `wikelo-data.json` template to include in your repository so users know how to format their items and recipes?**
